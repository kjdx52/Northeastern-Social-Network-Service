package com.Group11.NortheasternSocialNetwork.controller;

import com.Group11.NortheasternSocialNetwork.model.Post;
import com.Group11.NortheasternSocialNetwork.service.PostService;
import org.springframework.web.bind.annotation.*;

import java.util.Date;
import java.util.List;
import java.util.Map;

@CrossOrigin(value="http://localhost:3000")
@RestController
@RequestMapping("/api/v1/post")
public class PostController {

    private PostService postService;

    // Constructor to inject PostService dependency
    public PostController(PostService postService) {
        this.postService = postService;
    }

    // Endpoint for adding a new post
    @PostMapping
    public Post addPost(@RequestParam Map<String,String> requestParams) throws Exception {

        // Extracting values from request parameters
        String strPost = requestParams.get("post");
        String email = requestParams.get("email");
        String name = requestParams.get("name");
        String file = requestParams.get("file");
        String profilePic = requestParams.get("profilePic");

        // Building a Post object using the Builder pattern
        Post post = Post.builder()
                .file(file)
                .name(name)
                .email(email)
                .post(strPost)
                .profilePic(profilePic)
                .timeStamp(new Date().toString())
                .build();

        // Adding the post using the PostService
        post = postService.addPost(post);

        // Returning the added post
        return post;
    }

    // Endpoint for retrieving all posts
    @GetMapping
    public List<Post> getPost(){
        // Retrieving and returning the list of posts using the PostService
        return postService.getPost();
    }
}
